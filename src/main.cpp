...
uint256 hashGenesisBlock("0x000006a3be5567e3c259e254accbb6e089e43ed2f713c8569e155db02c38db22");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KazukiCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KazukiCoin Signed Message:\n";
...
{
    // KazukiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // KazukiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "KazukiCoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1482723131;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1161592;
        ...
        assert(block.hashMerkleRoot == uint256("0x660ee4c114613d368a33d57d1874cd82adebb65ceb22e1bd1a761bfbe7f7e72d"));
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc6, 0xbc, 0xde }; // KazukiCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KazukiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KazukiCoinMiner : ProcessBlock, block not accepted");
    }
